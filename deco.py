...
class DecoZHL16:
    ...
    def update_asc_desc(self, time, p_begin, p_end, f_n2, f_he):
        p_begin_alv_n2 = self.dtype((p_begin - self.p_h2o) * f_n2)
        p_begin_alv_he = self.dtype((p_begin - self.p_h2o) * f_he)
        R_n2 = self.dtype(((p_end - self.p_h2o) * f_n2 - p_begin_alv_n2) / time)
        R_he = self.dtype(((p_end - self.p_h2o) * f_he - p_begin_alv_he) / time)
        
        for i in range(0, self.n_tissues):
            # Update tissues gas loadings
            k = self.dtype(numpy.log(2) / self.halftime_n2[i])
            self.p_tissues_n2[i] = p_begin_alv_n2 + R_n2 * (time - 1 / k) \
                - (p_begin_alv_n2 - self.p_tissues_n2[i] - (R_n2 / k)) \
                * numpy.exp(-k * time)
            
            k = self.dtype(numpy.log(2) / self.halftime_he[i])
            self.p_tissues_he[i] = p_begin_alv_he + R_he * (time - 1 / k) \
                - (p_begin_alv_he - self.p_tissues_he[i] - (R_he / k)) \
                * numpy.exp(-k * time)
    ...
    def gf_current_get(self, p_ambient):
        gf = ((self.p_tissues_n2 + self.p_tissues_he - p_ambient) / ((p_ambient / self.b + self.a) - p_ambient)).max()
        if (gf < 0):
            return 0
        return gf
    ...
    def decotime_update(self, p_ambient, f_n2, f_he):
        ...
        p_ceiling = self.dtype(temp_ctx.ceiling_get_pressure(self.gf_low))
        ...
        while (stop_depth >= self.last_stop):
            ...
            if (p_ceiling < self.depth_to_pressure(clearance_depth)):
                ...
                if (stop_time == 0):
                    ...
                    temp_ctx.update_constant(
                        time = 1 - self.stop_size / self.ascend_speed,
                        p_ambient = self.depth_to_pressure(stop_depth),
                        f_n2 = f_n2,
                        f_he = f_he)
        ...
        if (p_ambient > self.depth_to_pressure(self.stops[0]['depth'])):
            ...
            time = (self.pressure_to_depth(p_ambient) - self.stops[0]['depth']) \
                / self.ascend_speed
            
            # Add initial ascent from ambient to first stop.
            self.time_to_surface += numpy.ceil(time)
